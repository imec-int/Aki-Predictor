...
## extra tables needed for MIMIC database

the sql [2\_pivot_weigths.sql](./sql/mimicIII/2_pivot_weights.sql) makes use of an extra table called echo_data. This can be generated by executing the code [here](https://github.com/MIT-LCP/mimic-code/tree/main/mimic-iii/concepts#generating-the-concepts-in-postgresql-nixmac-os-x).

I had to adopt the scripts as well, as they're linux focussed and regex on Mac OS works differently than on Linux, e.g. 

| original linux | adapted Mac OS |
| -------------- | -------------- |
| sed -r         | sed -E         |
| (.+?)          | (.+)           |

This script then creates extra tables under "public" on the database, among which echodata.