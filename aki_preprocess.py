...
def get_aki_patients_7days(cfg: config, aki_sql_results, aki_out_dataset, debugprint: bool, label="", printfile=None):
    '''
    ...
    if(debugprint):
        print("{}:admissions info:".format(label, df.shape), file=printfile)
        print("{}:number of unique subjects in admission:".format(label,
              df['SUBJECT_ID'].nunique()), file=printfile)
        print("{}:number of icustays info in admissions:".format(label,
              df['ICUSTAY_ID'].nunique()), file=printfile)
        print("{}:the biggest number of ICU stays for a patient:".format(label,
              info_save['COUNTTIMESGOICU'].max()), file=printfile)

    c_aki_7d = read_queried(cfg, aki_sql_results)
    if(debugprint):
        print("{}:Total icustays:{}".format(
            label, c_aki_7d['ICUSTAY_ID'].nunique()), file=printfile)
        print("{}:NORMAL Patients in 7DAY:{}".format(label,
                                                     c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 0]['ICUSTAY_ID'].count()), file=printfile)
        print("{}:AKI patients STAGE 1 within 7DAY:{}".format(label,
                                                              c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 1]['ICUSTAY_ID'].count()), file=printfile)
        print("{}:AKI Patients STAGE 2 in 7DAY:{}".format(label,
                                                          c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 2]['ICUSTAY_ID'].count()), file=printfile)
        print("{}:AKI Patients STAGE 3 7DAY:{}".format(label,
                                                       c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 3]['ICUSTAY_ID'].count()), file=printfile)
        print("{}:NAN patients within 7DAY:{}".format(label,
                                                      c_aki_7d['AKI_STAGE_7DAY'].isna().sum()), file=printfile)
    c_aki_7d = c_aki_7d.dropna(subset=['AKI_STAGE_7DAY'])
    if(debugprint):
        print("{}:Total icustays:".format(label,
              c_aki_7d['ICUSTAY_ID'].nunique()), file=printfile)
    ...
    for temp in icustays_data:
        ...
        df_corresponds_check = df_save['ICUSTAY_ID'] == int(
            temp['ICUSTAY_ID'].values[0])
    ...
    if(debugprint):
        # print(label,': NORMAL Patients in 7DAY: {}'.format(
        #     c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 0]['ICUSTAY_ID'].count()), file=printfile)
        # print(label,':AKI patients STAGE 1 within 7DAY: {}'.format(
        #     c_aki_7d.loc[c_aki_7d['AKI_STAGE_7DAY'] == 1]['ICUSTAY_ID'].count()), file=printfile)
        print('{}:CKD counted as normal:{}'.format(label,
                                                   count_ckd_normal), file=printfile)
        print('{}:CKD counted as aki:{}'.format(label,
                                                count_ckd_aki), file=printfile)
        print('{}:AKI on admission counted as normal:{}'.format(label,
                                                                count_akibefore_normal), file=printfile)
        print('{}:AKI on admission counted as aki:{}'.format(label,
                                                             count_akibefore_aki), file=printfile)
        print('{}:RENAL FAILURE counted as normal:{}'.format(label,
                                                             count_renalfailure_normal), file=printfile)
        print('{}:RENAL FAILURE counted as aki:{}'.format(label,
                                                          count_renalfailure_aki), file=printfile)
        print('{}:normal:{}'.format(label, count_normal), file=printfile)
        print('{}:aki:{}'.format(label, count_aki), file=printfile)
...
def main(cfg, printfile=None):
    ...
    c_aki_7d = c_aki_7d.dropna(subset=['AKI_STAGE_7DAY'])
    print("USING URINE AND CREATININE")

    print("creat+urine:Total icustays (AKI_KIDIGO_7D_SQL):",
          c_aki_7d['ICUSTAY_ID'].nunique(), file=printfile)
    ...
    print("creat:Total icustays (AKI_KIDIGO_7D_SQL):",
          c_aki_7d['ICUSTAY_ID'].nunique(), file=printfile)
    ...
    print("INSIGHTS USING URINE AND CREATININE")
    get_aki_patients_7days(cfg, 'AKI_KIDIGO_7D_SQL.parquet',
                           'INFO_DATASET_7days_creatinine+urine2.parquet', debugprint=True, label="creat+urine", printfile=printfile)
    # get aki patients 7 days merged with only creatinine
    print("INSIGHTS USING ONLY CREATININE")
    get_aki_patients_7days(cfg, 'AKI_KIDIGO_7D_SQL_CREATININE.parquet',
                           'INFO_DATASET_7days_creatinine2.parquet', debugprint=True, label="creat", printfile=printfile)
