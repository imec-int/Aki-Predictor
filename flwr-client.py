...
class config:
    def __init__(self, dbname, input_filename, output_foldername, df_columns) -> None:
        self.dbname = dbname
    ...
    def input_path(self) -> Path:
        return Path.cwd() / "data" / self.dbname / "preprocessed" / self.input_filename

    def log_path(self) -> Path:
        return Path.cwd() / "data" / self.dbname / "model" / "logs" / self.output_foldername / self.now.strftime("%Y%m%d-%H%M%S")
...
class FlwrClient(fl.client.NumPyClient):
    ...
    def get_data(self):
        input_pth = self.config.input_path()
        df = pd.DataFrame()
...
config_creat_no_urine = config("eicu", "INFO_DATASET_7days_creatinine2.csv", "creatinine_model", 
    [
        "AKI",
        "AKI_STAGE_7DAY",
        "CREATININE_MAX",
...
config_creat_urine = config("eicu", "INFO_DATASET_7days_creatinine+urine2.csv", "creatinine_urine_model", 
    [
        "AKI",
        "AKI_STAGE_7DAY",
        "UO_RT_24HR",
...
if __name__ == "__main__":
    ...
    config = config_creat_urine #default
    config.dbname = dbname

    fl.client.start_numpy_client(server_address="[::]:8080", client=FlwrClient(config))