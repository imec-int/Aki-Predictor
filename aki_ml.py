...
def cleanup_data(filename, outdir):
    ...
    df = pd.read_csv(
        open(Path(Path.cwd() / 'preprocessing' / filename), "r"), delimiter=",")
...
def cluster_ethnicity(df):
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(256, activation='relu'),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Dense(to_categorical(
    ...
    compute_metrics(Y_others_non_caucasian,
                    Y_others_non_caucasian_pred, multiclass=True)
...
def split_randomly(df, ratio, str):
    ...
    Y_pred_train, Y_pred_test = aki_model(X_train, to_categorical(
        Y_train), X_test, to_categorical(Y_test), str)
...
if __name__ == '__main__':
    ...
    df = cleanup_data("INFO_DATASET_7days_creatinine2.csv", outdir)
    ...
    df2 = cleanup_data("INFO_DATASET_7days_creatinine+urine2.csv", outdir)
